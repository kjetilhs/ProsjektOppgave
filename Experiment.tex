...
The landing was perform by angel the \gls{uav} towards the wind direction. The \gls{rtk-gps} position estimate during the landing is shown in figure \ref{figure:landingNorthEastFlight}, as well as the integer solution type. The \gls{rtk-gps} system was unable to maintain a fixed solution, however it did maintain a float solution. Further test with a control system is required in order to test if also the float solution is accurate enough to perform a automatic landing.
\begin{figure}[H]
	\centering
